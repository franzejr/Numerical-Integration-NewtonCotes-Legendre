...
class Simpson38(NewtonCotes):
	...
	def resolver(self,a,b,n):
		...
		for i in range(0,n):
			...
			integral += f(x0) + 3*f(x1) + 3*f(x2) + f(x3)
			a = b
...
class Simpson38Aberto(Simpson38):
	...
	def resolver(self,a,b,n):
		...
		for i in range(0,n):
			...
			integral += 11*f(x1) + f(x2) + f(x3) + 11*f(x4)
			ai = bi
...
class BooleAberto(Boole):
	...
	def resolver(self,a,b,n):
		...
		for i in range(0,n):
			...
			integral += 11*f(x1) - 14*f(x2) + 26*f(x3) -14*f(x4) + 11*f(x5)
			ai = bi
